<template>
  <div class="maintenance-readiness">
    <v-card>
      <v-card-title class="bg-primary text-white">Maintenance Readiness Report</v-card-title>
...
const sub = (item) => item.subName || item.sub || ''
...
const props = defineProps({ obj: { type: Object, default: () => ({ master: [], result: {} }) } })

const mock = {
  master: [
    { title: true, category: 'MOBILITY', sub: 'MOBILITY' },
    { title: false, category: 'MOBILITY', sub: 'ENGINE', subName: 'Engine Assemblies', required: 8, actual: 7 },
    { title: false, category: 'FIREPOWER', sub: 'AMMO', subName: 'Ammunition', required: 200, actual: 190 }
  ],
  result: { readiness: 88.12 }
}
...
<style scoped>
.maintenance-readiness { padding: 1rem }
...
.maintenance-readiness :deep(thead th) {
...
@media (max-width: 960px) {
  ...
  .maintenance-readiness :deep(thead th) { padding: 12px 4px !important; font-size: 0.75rem !important }
}
</style>
